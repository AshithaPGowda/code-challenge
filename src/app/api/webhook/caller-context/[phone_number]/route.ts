...
export async function GET(
  ...
  try {
    const { phone_number } = await context.params;
    const { searchParams } = new URL(request.url);
    const call_control_id = searchParams.get('call_control_id');

    // Decode the phone parameter (handles URL encoding like %2B for +)
    const phone = phone_number ? decodeURIComponent(phone_number) : null;

    console.log(`[Webhook] Raw phone param: "${phone_number}", decoded: "${phone}"`);
